...
(defn render [icon & [{:keys [size color style]}]]
  (if-let [svg (get @icons icon)]
    (assoc-in svg [1 :style] (cond-> {:display "inline-block"
                               ...
                               style (into style)))
    (throw (js/Error. (str "Icon " icon " is not loaded. Try loading it with `load-icon!`, or check that it exists.")))))
